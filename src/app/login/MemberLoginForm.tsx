...
export default function MemberLoginForm({ next }: { next: string }) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-5">
      ...
      <button
        type="submit"
        disabled={pending}
        className="mt-2 flex h-12 items-center justify-center gap-2.5 rounded-xl bg-forest text-sm font-semibold text-cream transition-colors hover:bg-forest-deep disabled:opacity-60"
      >
        {pending && (
      ...
      <p className="text-center text-sm text-ink/65 mt-2">
        Don&apos;t have an account?{" "}
        <Link href="/join" className="text-forest font-semibold hover:underline">
          Join us
        </Link>
      </p>
    </form>
